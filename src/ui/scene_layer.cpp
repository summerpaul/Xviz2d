...
namespace ui
{
    ...
    void SceneLayer::Draw()
    {
        ...
        drawList->AddRectFilled(view_rect_min, view_rect_max, options->backgroundColor);
    ...
    void SceneLayer::HandleZoomScroll(const ImGuiIO &io)
    {
        ...
        Vec2f targetOrigin = origin - deltaZoom * invScalePre * (mousePositionScene - origin);
                                  ...
                                  ImGuiComboFlags_NoArrowButton))
            {
                for (auto &frameId : m_scene->GetFrameIdList())
                {
                    ...
                    if (ImGui::Selectable(frameId.second.data(), is_selected))
                    {
                        ...
                        auto tf_tree = m_scene->GetTFTree();
                        if (tf_tree->CanTransform(m_scene->GetDrawFrameID(), frameId.second))
                        {
                            auto tf = tf_tree->LookupTransform(frameId.second, m_scene->GetDrawFrameID());
    ...
    void SceneLayer::HandleMeasure(const ImGuiIO &io)
    {
        ...
        if (io.MouseDown[0])
        {
            ...
            else
            {
                ...
                drawList->AddText(text_pose,
                                  options->measureColor,
